...
CV=ceil(0.2*m);
U0=T{1:training,2};
U=(T{1:training,4:19}).^2;
U1=(T{1:training,20:21}).^3;
U2=U.^4;
U3=sqrt(U1.^3);
X=[ones(training,1) U U1 U2 U3]; 
n=length(X(1,:));
%Scaling of x
for w=2:n
    if max(abs(X(:,w)))~=0
...
U_cv=(T{training+1:training+CV,4:19}).^2;
U1_cv=(T{training+1:training+CV,20:21}).^3;
U2_cv=U_cv.^4;
U3_cv=sqrt(U1_cv.^3);
...
U_test=(T{training+CV+1:end,4:19}).^2;
U1_test=T{training+CV+1:end,20:21}.^3;
U2_test=U_test.^4;
U3_test=sqrt(U1_test.^3);
